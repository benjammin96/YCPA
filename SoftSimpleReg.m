...
mu{1} = ones(nx{1},ny{1})*c_mu_0;
 ...
 epi{1} = ones(nx{1},ny{1})*c_eps_0; %% permitivittys 
 epi{1}(125:150,55:95)= c_eps_0*11.3;
 epi{1}(50:70,45:54)= c_eps_0*5;%% added a permitivity
 epi{1}(40:49,35:44)= c_eps_0*5;%% added a permitivity
...
bc{1}.NumS = 2; %% two soft sources
bc{1}.s(1).xpos = nx{1}/(4) + 1; % position of source
bc{1}.s(1).type = 'ss'; %% soft type
bc{1}.s(1).fct = @PlaneWaveBC; 

bc{1}.s(2).xpos = nx{1}/(4) + 40; %% position of source
bc{1}.s(2).type = 'ss'; %% soft type
